...
>
> CAUTION: **On a temporary instrument.** The sandbox links exactly one phone number,
...
- [ ] ★ Meta developer account, app created, WhatsApp product added — **Yuri**.
      CAUTION: **Blocked since 2026-08-04 on Meta's signup SMS, which is a documented
...
- [ ] ★ Provision the number:
      ...
      the steps above.** CAUTION: Not plain `node`; see `docs/03-RESOURCES.md` §6.
...
- [x] **Keyword search — Postgres full-text over `body_text`** (2026-08-02).
      ...
      **CAUTION: The config is `simple`, not `english`, and that is deliberate** —
      ...
      **CAUTION: Two query modes on purpose.** `to_tsquery` RAISES on malformed input
...
- [x] **Search results carry their AI summary** (2026-08-02) — migration `0010`.
      ...
      CAUTION: `create or replace function` **cannot change a RETURNS TABLE**, so 0010
      ...
      reports success.
      CAUTION: **LEFT join, and the `kind` filter must sit in the JOIN condition, not
...
- [x] **Contact list; contact detail = merged cross-channel history** (2026-08-03)
      ...
      `soon` nav item on.
      **CAUTION: The detail view is CONVERSATIONS, not "messages they sent".**
      ...
      **CAUTION: Identities are listed, never collapsed.** With one channel connected
      ...
      CAUTION: It found a real defect in `initials()`: a `display_name` that **is** a
...
- [x] **Manual identity merge** — *"this email address and this number are the
      ...
      **starts closed**, offers ranked suggestions, preselects nothing, and
      ...
      button.
      **CAUTION: Manual, and it stays manual.** Q3: *"Same display name across channels
...
- [ ] Responsive down to mobile width
      *(verified for search at 375px: no page-level horizontal scroll, all 24
      ...
      CAUTION: Measure with transitions disabled — `transition-colors` means
...
**CAUTION: This is a THIRD AI workload, not a variation of the other two.** Getting that
...
- [x] `packages/ai` — `CompletionProvider` over **Groq** (ADR-003).
      **CAUTION: Model verified from the live rate-limit headers, 2026-08-02, not from
      ...
      CAUTION: **Deviation from `docs/02-ARCHITECTURE.md` §8, stated not smuggled:**
...
- [x] **Skip rules, applied before spending a request.** `empty` and
      `already-short` (< 280 chars). CAUTION: **Consequence worth saying out loud
...
- [x] **Backfill the existing corpus** — `apps/worker/scripts/backfill-summaries.ts`.
      ...
      failures.
      CAUTION: **Running it found the real limit, which is not the documented one.**
      ...
      honouring `retry-after` (CAUTION: fractional — `parseFloat`). Full note in
...
- [x] **Taglish — pinned: the summary is ALWAYS English.** A Tagalog-dominant
      ...
      CAUTION: **The two rules interact:** translating necessarily introduces words not
...
- [x] **Console:** renders in the **opened row**, above the body, in **IBM Plex
      ...
      CAUTION: **The left rule is NEUTRAL, not amber.** It was `border-live/40` for one
      ...
      CAUTION: **The PostgREST embed must stay a LEFT join.** `extractions!inner`
...
- [x] **Eval set** — `apps/worker/scripts/eval-summaries.ts`, 10 cases,
      ...
      CAUTION: Two bugs it found were in the *harness*, and both are instructive:
      ...
      CAUTION: **Residual weakness, recorded honestly:** an injection claiming *"the
...
> CAUTION: **The deployed worker runs an image pinned by DIGEST, and CI does not
...
> CAUTION: **This is the first time message content leaves the system.** Until now
...
- [x] **e5 prefixes** — applied *inside* `embedQuery` / `embedPassages`, never
      ...
      ranking.
- [x] Chunking with overlap → `message_chunks`. CAUTION: Two real bugs the tests caught:
...
- [x] Prompt: answer strictly from context, cite message ids, refuse otherwise.
      CAUTION: **Three-way as of 2026-08-02 (ADR-017)** — "nothing here is about this"
...
- [x] **Citations link to `/messages/[id]`** (ADR-018) — a signed-in, RLS-scoped
      route rendering one message in full. CAUTION: **Not** a jump into the timeline as
...
- [x] **Unit tests for the refusal contract** — `packages/ai/test/assistant.test.ts`.
      CAUTION: Until 2026-08-02 **none of the project's 356 tests touched
...
> ### CAUTION: Where the eval actually stands — REWRITTEN 2026-08-02, see ADR-017
>
...
> held.
>
> CAUTION: Scores are reported as **two numbers, never one.** A combined figure reads
...
>
> CAUTION: Some eval runs still fail on `groq rate limit`. That is the **daily token
...
> ### CAUTION: The assistant no longer runs on Gemini
>
...
>
> **CAUTION: It crashlooped first, with exit code 137 — OOM.** The worker was sized
...
>
> CAUTION: Kept because they are needed again on any new Vercel project or preview
...
>
> CAUTION: **Do not attempt to fix either in the assistant's prompt.** That has been
...
- [x] **Extraction pass in worker** (2026-08-03): commitments, meetings, action
      ...
      **CAUTION: It must NEVER fail an event.** Sits between ingest and `markDone`,
      ...
      **CAUTION: Idempotency needed a new table — migration 0011, ADR-019.**
      ...
      **CAUTION: The quote check is the hallucination guard.** Every item must carry the
      ...
      **CAUTION: Relative dates resolve against the message's SEND time**, never today.
      ...
      CAUTION: Two measurements worth keeping: one request costs **~3,500 tokens**
      ...
      **`max_tokens: 900` was too low** — two Taglish messages returned 1,798
      ...
      CAUTION: It was only diagnosable because `validateExtractions` reports "ended
      mid-structure" separately from "answered in prose" — the same words, two
      opposite fixes.
      **CRITICAL: Two quality failures were found by RUNNING it, and both are recorded
      ...
      *Injections:* the first eval run scored **8/10, and both failures were
      ...
      approval of the 2M budget"*. CAUTION: **The quote check cannot catch this**: the
      ...
      both. CAUTION: Deliberately **not** solved by pattern-matching for injections:
      ...
      **Measured after both fixes, over the whole corpus: 77/77 messages, 0
      ...
      CAUTION: **One residual weakness, recorded rather than hidden** — the same way
...
- [x] **"Needs attention" view over extractions** (2026-08-03) — `/attention`,
      ...
      **CAUTION: Every row shows the sentence it came from**, on the row, not behind a
      ...
      CAUTION: **Measure `lab()` colours, not `rgb()`.** This console's computed
...
- [x] **Calendar write-back (US-7b, ADR-010)** (2026-08-03):
  ...
  - [x] **CAUTION: Attendees are deliberately NOT sent to Google.** Adding one makes
...
- [x] **Extraction catch-up sweep** (2026-08-03) — `apps/worker/src/extract-catchup.ts`.
      CAUTION: **This closed a hole that was losing live mail, found by counting rather
      ...
      they share. CAUTION: An inline retry was rejected deliberately: it would block
...
- [x] ~~Daily digest (US-9)~~ **CUT 2026-08-03 by Yuri (R25).** `/attention`
      ...
      CAUTION: Recorded as a decision rather than left unticked, so it does not read as
      outstanding work.
...
- [x] **`README.md` verified from a clean clone** (2026-08-03) — cloned to an
      ...
      CAUTION: **Step 4 did not, and it was the README's headline claim.** *"Nothing
...
- [ ] **Demo rehearsal on deployed infrastructure, not localhost**
      CAUTION: **Steps 3–5 are blocked on Meta**, and step 7 has a problem found
...
- [x] **Channel filter on the timeline** — *"Gmail lang or WhatsApp lang… para
      ...
      CAUTION: The URL carries channel *types*, the query takes channel *ids*; ids in
...
- [x] **`/attention` becomes a Kanban board** — *"not started, in progress,
      ...
      CAUTION: `confirmed_at` is not "done"; Done sorts by `status_changed_at`.
