            ...
            */}
            {message.summary && (
              <div className="mb-3 border-l-2 border-border pl-3">
                ...
                <p className="max-w-[62ch] font-mono text-note text-muted-foreground [overflow-wrap:anywhere]">
            ...
            {/*
              ⚠ `[overflow-wrap:anywhere]` ALONGSIDE `whitespace-pre-wrap`, and
              `break-words` is not a substitute. Same reasoning as the attention
              board, and this is where it was found the hard way: a LinkedIn job
              alert carries a ~400-character tracking URL with no whitespace in
              it, `pre-wrap` has nowhere to break, and the line ran off the
              right of the screen and gave the whole PAGE a horizontal
              scrollbar. `max-w-[62ch]` caps the box, not what overflows it.

              `break-words` would not have fixed it either: it only breaks a
              word that would overflow on a line of its own, and this URL sits
              after ordinary words. `anywhere` also counts as a break
              opportunity when the browser computes min-content width, which is
              what stops the column widening in the split view.
            */}
            {body.text ? (
              <p className="max-w-[62ch] text-row whitespace-pre-wrap [overflow-wrap:anywhere]">
                {body.text}
              </p>
            ) : (
              /*
